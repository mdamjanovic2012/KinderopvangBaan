...
Importeert alle diploma's van de kinderopvang-werkt.nl API (~1020 diploma's).
...
Rollback: als het aantal diploma's na de import daalt, wordt de transactie teruggedraaid.
...
def _last_run_date() -> date | None:
    try:
        return date.fromisoformat(_SENTINEL.read_text().strip()[:10])
    except Exception:
        return None
...
class Command(BaseCommand):
    help = "Diploma-update via kinderopvang-werkt.nl API (max 1x per 180 dagen)"
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"Vorige run: {last} ({days_ago} dagen geleden)" if last else "Vorige run: nooit")
        ...
        if options["dry_run"]:
            ...
            self.stdout.write("=== Dry-run klaar — niets opgeslagen ===")
            return

        # ── Import in één atomaire transactie ────────────────────────────────
        # Rollback als aantal diploma's daalt na de import.
        try:
            with transaction.atomic():
                ...
                if count_after < count_before:
                    raise ValueError(
                        f"DIPLOMA_ROLLBACK: aantal gedaald van {count_before} naar "
                        f"{count_after}. Alle wijzigingen teruggedraaid."
                    )

                self.stdout.write(self.style.SUCCESS(
                    f"Diploma's na import: {count_after} (+{added} nieuw)"
                ))
